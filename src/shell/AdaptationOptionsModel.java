...
public class AdaptationOptionsModel extends AbstractTableModel {
	...
	//temporary fix for the bug where ao model keeps a record of voting result 
	public void purgeVotingColumns(){
	// remove 4 columns
	   
	   if (colNames.contains("points")) {
		   colNames.remove("points");
		   removeColumn("points");
		   columnLengths.remove(dataColumns-1);
	   }
	   if (colNames.contains("score")) {
		   colNames.remove("score");
		   removeColumn("score");
		   columnLengths.remove(dataColumns-2);
	   }
	   if (colNames.contains("rank")) {
		   colNames.remove("rank");
		   removeColumn("rank");
		   columnLengths.remove(dataColumns-3);
	   }
	   if (colNames.contains("voter")) {
		   colNames.remove("voter");
		   removeColumn("voter");
		   columnLengths.remove(dataColumns-4);
	   }
	}
	...
	public void removeColumn(String strID){
		
		//Object obj = doc.getProperty(strID);
		root = doc.getRootElement();
		root.removeAttribute(strID);
	}
